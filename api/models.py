...
class KnowledgeDocument(Base):
    ...
    data_class: Mapped[str]=mapped_column(String(32),default="KB_POLICY",index=True)
    ...
    country: Mapped[str|None]=mapped_column(String(32),nullable=True,index=True)
    ...
    answerable: Mapped[bool]=mapped_column(Boolean,default=True,index=True)
    authority_level: Mapped[str]=mapped_column(String(32),default="community",index=True)
    ...
    minimization_rule: Mapped[str|None]=mapped_column(String(500),nullable=True)
    parser_version: Mapped[str]=mapped_column(String(64),default="structured-v1")
    review_status: Mapped[str]=mapped_column(String(32),default="approved",index=True)
    translation_provider: Mapped[str|None]=mapped_column(String(100),nullable=True)
...
class KnowledgeSection(Base):
    ...
    # Indexed content is immutable per controlled document version.
    document_version: Mapped[str]=mapped_column(String(64),default="1.0",index=True)
...
class KnowledgeChunk(Base):
    ...
    created_at: Mapped[datetime]=mapped_column(DateTime,default=now)
    __table_args__=(UniqueConstraint("document_id","document_version","chunk_index",name="uq_knowledge_chunk_document_version_index"),)
...
class RagQueryLog(Base):
    ...
    product_mode: Mapped[str]=mapped_column(String(32),default="domestic_beijing",index=True)
    resolved_jurisdiction: Mapped[str|None]=mapped_column(String(120),nullable=True,index=True)
