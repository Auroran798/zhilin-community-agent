...
Write-Host "[1/4] 便携 Python 和依赖检查" -ForegroundColor Cyan
if (-not (Test-Path -LiteralPath $python)) { throw "缺少 runtime\python.exe。" }
$version = & $python -c "import sys; print(sys.version.split()[0]); import fastapi,streamlit,sqlalchemy,chromadb; print('runtime-imports-ok')"
if ($LASTEXITCODE -ne 0) { throw "便携 Python 依赖导入失败。" }
$version
...
if ($ready.rag_quality.mode -ne "offline_fallback") { throw "RAG 质量模式与提交版说明不一致。" }
...
Write-Host "`nPASS：便携 Python、API、离线 RAG、默认北京模式、数据边界和 Web 均正常。" -ForegroundColor Green
